...
if args.hist:
    ...
    # plt.plot(xgrid, negDist(xgrid), '-r', linewidth=4, color='lightgrey')
    # color_map= ['Brown', 'Green']
    #for gene in ref_list[i], color_map[i]:
        #plt.plot(xgrid, hitDist(xgrid), '-b', linewidth=4, color=color_map[0,1])
        #plt.plot(xgrid, hitDist(xgrid), linewidth=4, color="blue")
    if args.ylim:
    ...
    for gene, ref, effect in ref_list:
        ...
        for gene, ref, effect in ref_list:
            hitDist = st.gaussian_kde(enrich)
            plt.plot(xgrid, hitDist(xgrid), linewidth=4, color="black")
            plt.vlines(enrich, 0, ymax / 5.0, color='red')
            plt.axvline(x=effect, color='red', ls='dashed', linewidth=2)
            plt.axvline(x=0, color='lightgrey', ls='dashed', linewidth=2)

        plt.legend(['Safe distribution', + gene + ' distribution'])
        plt.xlabel('Enrichment')
        plt.ylabel('Frequency')

    plt.savefig(file_out + '_' + '_hist.' + args.file_type)
    plt.close()
