...
if args.hist:
    plt.hist2d(x, y, 200, cmap=cm.Greys, norm=LogNorm())

else:
    plt.plot(x, y, '.', alpha=0.25, markersize=10, markeredgewidth=0)
...
if args.names:
    for name, gene in gene_labels:
        effect, rat = gene2effect_rat[gene]
        plt.text(effect, rat, name, color='black', size=10)
        plt.plot(effect, rat, '.', color='red', markersize=6, markeredgewidth=0)
...
if args.thresh:
    ...
    plt.plot(x_thresh, y_thresh, '.', color='grey', markersize=4, markeredgewidth=0)
