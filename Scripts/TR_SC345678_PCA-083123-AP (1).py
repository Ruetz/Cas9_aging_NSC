...
output = "/Users/angiepogson/Dropbox/TR_Cas9Paper_CodeCheck/results/PCA083123/"

path = "./"

data1 = path+'SC3_RA_FDR0.1.csv'
data2 = path+'SC3_Kp_FDR0.1.csv'
data3 = path+'SC4_RA_FDR0.1.csv'
data4 = path+'SC4_Kp_FDR0.1.csv'
data5 = path+'SC5_RA_FDR0.1.csv'
data6 = path+'SC5_Kp_FDR0.1.csv'
data7 = path+'SC6_RA_FDR0.1.csv'
data8 = path+'SC6_Kp_FDR0.1.csv'
data9 = path+'SC7_RA_FDR0.1.csv'
data10 = path+'SC7_Kp_FDR0.1.csv'
data11 = path+'SC8_RA_FDR0.1.csv'
data12 = path+'SC8_Kp_FDR0.1.csv'
...
final = concat.to_csv(output + 'castlescores083123.csv')
...
final = concat.to_csv(output + 'castleEffect083123.csv')
...
final = concat.to_csv(output + 'SC345678_allgenescores_d14083123.csv')


Adata = pd.read_csv(output + 'castlescores083123.csv', header=None, skiprows=1, names=['#GeneID', 'Symbol', stdata1, stdata3, stdata5, stdata7, stdata9, stdata11])
tAdata = Adata.transpose()
wtAdata = tAdata.to_csv(output + 'castlescores_transpose083123.csv')
rdata = pd.read_csv(output + 'castlescores_transpose083123.csv', skiprows=1, header=1)
print(rdata.head())
...
################################################################################################################
#Pull out PC Loadings and put them into a table
#PC Loadings (PC1 and 2) for Day 14 is the 2nd sheet of Supp Table 5
################################################################################################################
loadings = pd.DataFrame(pca.components_.T, columns = ['PC1', 'PC2'], index = features)
print(loadings)
loadings.to_csv(output + 'PCloadings_d14.csv')
################################################################################################################
...
# print('explained varaince = ', explained_variance)
# 
...
# 
# ##ax.grid()
# plt.savefig(output + 'SC345678_PCA_d14_age_PC12_score083123', dpi=300)
...
principalComponents = pca.fit_transform(x)
################################################################################################################
#This is the one line of code that I needed to add to pull out PC loadings
#PC Loadings (PC1,2,3 and 4) for Day 4 is the 3rd sheet of Supp Table 5
################################################################################################################
loadings_d4 = pd.DataFrame(pca.components_.T, columns = ['PC1', 'PC2', 'PC3', 'PC4'], index = features)
print(loadings_d4)
loadings_d4.to_csv(output + 'PCloadingsd4.csv')
#To explain the total variance (without accounting for direction) of the PCs of Day 4, take the absolute value
loadings_d4.abs().to_csv(output + 'PCloadingsd4_abs_val.csv')
################################################################################################################
# principalDf = pd.DataFrame(data = principalComponents, columns = ['principal component 1', 'principal component 2', 'principal component 3', 'principal component 4'])
# 
# print(principalDf)
