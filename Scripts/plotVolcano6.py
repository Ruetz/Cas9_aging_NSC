...
if args.hist:
    plt.hist2d(x, y, 75, cmap=cm.Greys, norm=LogNorm())

else:
    plt.plot(x, y, '.', alpha=0.25, markersize=8, markeredgewidth=0)
...
if args.names:
    for name, gene in gene_labels:
        effect, rat = gene2effect_rat[gene]
        plt.plot(effect, rat, '.', color='black', markersize=10, markeredgewidth=0)
